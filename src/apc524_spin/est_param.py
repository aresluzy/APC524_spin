...
def initialize_lattice(L):
    """
    Initialize a 3D L×L×L XY-spin lattice with random angles.

    Each lattice site stores a 2D unit vector (cos θ, sin θ), so the returned array has shape ``(L, L, L, 2)``.

    Parameters
    ----------
    L : int
    Linear system size. The total number of sites is ``L**3``.

    Returns
    -------
    spins : ndarray of shape (L, L, L, 2), dtype float
    XY spins represented as unit vectors.

    Notes
    -----
    No validation is performed here; passing non-positive ``L`` will lead to a zero-sized or invalid array downstream.
    """
...
def wolff_update_new(spins, J, T):
    ...
    """
    Perform one Wolff cluster update (XY model) and return the cluster size.

    Uses a random reflection axis in the x–y plane and grows a cluster with
    bond-add probability
    :math:`p_{add} = 1 - \\exp(\\min(0, \\Delta))` where
    :math:`\\Delta = -2\\beta J (\\mathbf S_i\\cdot\\mathbf r)(\\mathbf S_j\\cdot\\mathbf r)`,
    reflecting the corrected sign for the XY Wolff algorithm.

    Parameters
    ----------
    spins : ndarray, shape (L, L, L, 2)
        Current XY-spin configuration (unit vectors).
    J : float
        Ferromagnetic coupling constant.
    T : float
        Temperature in the same units as ``J``; used via ``beta = 1/T``.

    Returns
    -------
    cluster_size : int
        Number of spins flipped in the constructed cluster.

    Raises
    ------
    FloatingPointError
        If ``T`` is zero (division by zero in ``beta = 1/T``) or NaN/Inf.
    ValueError
        If ``spins`` does not have the expected 4D shape ``(L, L, L, 2)``.
    """
...
def wolff_update_with_estimator(spins, J, T):
    """
    Run a Wolff-cluster Monte Carlo for the 3D XY model and collect observables.

    The routine equilibrates for ``n_equil`` Wolff updates, then performs
    ``n_steps`` measurement updates with energy, magnetization, and
    cluster-size tracking. Susceptibility, specific heat, and Binder cumulant
    are computed with simple uncertainty estimates.

    Parameters
    ----------
    L : int
        Linear system size; the volume is ``V = L**3``.
    J : float
        Ferromagnetic coupling constant.
    T : float
        Temperature (``beta = 1/T``).
    n_steps : int
        Number of measurement Wolff steps after equilibration.
    n_equil : int
        Number of Wolff steps for equilibration (no measurements recorded).
    ...
    results : dict
        ...
        - ``'magnetizations'`` : ndarray of shape (n_steps,)
            Per-step |M|/V values.
        - ``'energies'`` : ndarray of shape (n_steps,)
            Per-step energy per site.
        - ``'cluster_sizes'`` : list of float
            Cluster sizes normalized by volume per step.
        - ``'susceptibility'`` : tuple (chi, error_chi)
            Magnetic susceptibility estimate and its standard error.
        - ``'specific_heat'`` : tuple (C, error_C)
            Specific heat per site and its standard error.
        - ``'binder_cumulant'`` : tuple (U, error_U)
            Binder cumulant and its standard error.
        - ``'spin'`` : ndarray of shape (L, L, L, 2)
            Final spin configuration after the last update.

    Raises
    ------
    ValueError
        If ``n_steps`` or ``n_equil`` are negative or if ``L < 1``.
    FloatingPointError
        If an invalid temperature (e.g., ``T == 0``) causes division by zero.

    Examples
    --------
    Run a short simulation and check array lengths:

    results = run_simulation(L=8, J=1.0, T=2.2, n_steps=100, n_equil=50)
    len(results['magnetizations']) == 100
    True
    'binder_cumulant' in results and isinstance(results['binder_cumulant'], tuple)
    True
    """
    beta = 1.0 / T
    L = spins.shape[0]
...
def compute_improved_structure_factor(cluster_positions, L):
    """
    Compute the improved estimator of the structure factor S(q) from
    Wolff cluster geometry at the smallest nonzero momenta.

    Given the list of sites in a single cluster, this estimator evaluates
    ``S(q) = |∑_{r in cluster} e^{i q·r}|^2 / (3 * s)``, where ``s`` is the
    cluster size and the denominator averages over spin components for the
    XY model. The calculation is performed for the three lattice momenta
    ``q ∈ {(2π/L,0,0), (0,2π/L,0), (0,0,2π/L)}``.

    Parameters
    ----------
    cluster_positions : Sequence[tuple[int, int, int]] or ndarray, shape (s, 3)
        Lattice coordinates of the s sites (i, j, k) that belong to the
        current Wolff cluster.
    L : int
        Linear system size; positions are interpreted modulo L.

    Returns
    -------
    cluster_Sq : ndarray, shape (3,)
        Values of the improved structure factor S(q) for the three q-vectors.
    q_vectors : list of ndarray
        The three q-vectors used, each of shape (3,).

    Raises
    ------
    ValueError
        If ``L < 1`` or ``cluster_positions`` is empty.

    Notes
    -----
    The improved estimator reduces variance compared to measuring S(q)
    from full-lattice Fourier transforms, especially near criticality.

    Examples
    --------
    >>> L = 6
    >>> # toy cluster: three sites along x
    >>> cluster_positions = [(0,0,0), (1,0,0), (2,0,0)]
    >>> Sq, qvecs = compute_improved_structure_factor(cluster_positions, L)
    >>> Sq.shape, len(qvecs)
    ((3,), 3)
    >>> # S(q) is largest when q aligns with the cluster direction:
    >>> Sq[0] >= Sq[1] and Sq[0] >= Sq[2]
    True
    """
...
def compute_correlation_length_from_Sq(structure_factors, q_vectors, L):
    """
    Estimate the second-moment correlation length from low-q structure factors.

    Uses the isotropic second-moment definition
    ``xi = sqrt( mu_2 / (2 d mu_0) )``, where
    ``mu_0 = Σ_q S(q)`` and ``mu_2 = Σ_q |q|^2 S(q)`` over the provided
    small-momentum samples. This routine expects a few (≥3) nonzero |q|
    values such as the lattice momenta of magnitude 2π/L along the axes.

    Parameters
    ----------
    structure_factors : array_like, shape (m,)
        Measured S(q) at the m supplied q-vectors.
    q_vectors : array_like, shape (m, 3) or Sequence[ndarray]
        The corresponding q-vectors in lattice units.
    L : int
        Linear lattice size (currently unused in the calculation but kept
        for API consistency with other measurement utilities).

    Returns
    -------
    xi : float
        Estimated correlation length. Returns ``np.inf`` if the input
        moments are nonpositive (e.g., all zeros).

    Raises
    ------
    ValueError
        If the lengths of ``structure_factors`` and ``q_vectors`` differ
        or if fewer than one q-vector is supplied.

    Notes
    -----
    - Accuracy improves when including several distinct small |q| samples.
    - Ensure ``structure_factors`` correspond to the same simulation state
      (or averaged state) and that q’s are nonzero.

    Examples
    --------
    >>> import numpy as np
    >>> L = 8
    >>> q_vectors = [np.array([2*np.pi/L, 0, 0]),
    ...              np.array([0, 2*np.pi/L, 0]),
    ...              np.array([0, 0, 2*np.pi/L])]
    >>> S_q = np.array([10.0, 9.5, 10.2])   # toy low-q S(q)
    >>> xi = compute_correlation_length_from_Sq(S_q, q_vectors, L)
    >>> np.isfinite(xi)
    True
    """
...
# Computation Functions for critical components
...
def compute_binder_cumulant(magnetizations, V):
    """
    Compute Binder cumulant U and its standard error from |M| samples.

    Uses the definition
    :math:`U = 1 - \\langle M^4 \\rangle / (3 \\langle M^2 \\rangle^2)`,
    where the inputs are per-site absolute magnetizations, which are
    rescaled to total magnetization via ``M_abs = m * V``.

    Parameters
    ----------
    magnetizations : array-like of float
        Per-site absolute magnetizations |M|/V over Monte Carlo steps.
    V : int
        Volume of the system (``L**3``).
    ...
    U : float
        Binder cumulant.
    error_U : float
        Propagated standard error from sample variances of M² and M⁴.

    Raises
    ------
    ValueError
        If the input sequence is empty or its variance is undefined.
    """
    M_abs = np.asarray(magnetizations, dtype=float) * V
    if M_abs.size == 0:
        raise ValueError("magnetizations must be non-empty.")
    M2 = M_abs ** 2
    M4 = M_abs ** 4
    ...
    N = len(M_abs)
    if N < 2:
        raise ValueError("Need at least two samples to estimate error.")
...
def compute_specific_heat(energies, V, T):
    """
    Estimate specific heat per site and its standard error via blocking.

    The estimator uses the fluctuation formula and block-variance to
    approximate the uncertainty.

    Parameters
    ----------
    energies : array-like of float
        Per-site energies collected over Monte Carlo time (i.e., already
        divided by volume). Length ``N`` must be >= number of blocks used.
    V : int
        Volume of the system (``L**3``).
    T : float
        Temperature.
    ...
    sigma_C : float
        Standard error of the estimate (blocking over M=10 equal blocks).

    Raises
    ------
    ValueError
        If fewer than ``M`` samples are provided (so that block length is 0).

    Notes
    -----
    The computation assumes uncorrelated block means at the chosen block size
    (``M = 10``). For short time series or strong autocorrelation you should
    increase sampling or adjust M.
    """
    E = np.array(energies, dtype=float)
    N = len(E)
    if N < 10:
        raise ValueError("Need at least 10 energy samples to form 10 blocks.")
    E_mean = np.mean(E)
    E2_mean = np.mean(E ** 2)
    ...
    m = N // M
    if m == 0:
        raise ValueError("Block length is zero; increase number of samples.")
    variances = []
    for k in range(M):
        E_block = E[k * m:(k + 1) * m]
        E_mean_block = np.mean(E_block)
        E2_mean_block = np.mean(E_block ** 2)
        variances.append(E2_mean_block - E_mean_block ** 2)
